...
By default, the momentum equation does not account for turbulent Reynolds stresses. However, if the \texttt{turbulence\_parameterisation} option is enabled, then the Reynolds stresses can be parameterised through the calculation of an eddy viscosity, which models the effects of sub-grid-scale eddies on the large-scale flow turbulence. This eddy viscosity is added to the background viscosity $\nu$ in the stress term. More information can be found in Chapter \ref{chap:turbulence_parameterisation}.
